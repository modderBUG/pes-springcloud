    ...
    <div class="table-wrapper">
      <div class="tabel-container">
        <table>
          ...
          <tr>
            <th>时间</th>
            <th v-for="(weekNum, weekIndex) in courses.length" :key="weekIndex"> {{'周' + digital2Chinese(weekIndex, 'week')}}</th>
          </tr>
          </thead>
          <tbody>
          <tr v-for="(lesson, lessonIndex) in lessons" :key="lessonIndex">
            ...
            <td v-for="(course, courseIndex) in courses" :key="courseIndex">
              {{courses[courseIndex][lessonIndex] || '-'}}
            </td>
          </tr>
          </tbody>
        </table>
...
<script>
  ...
  export default {
    name: "TimeSchedule",  //这个组件被StuSchedule使用。

    props:['lessons','courses'],
    data() {
      return {
        classTableData: {
          lessons_: [
